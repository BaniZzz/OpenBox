...
while IFS='=' read -r key value; do
  case "$key" in
    ALIBABA_CLOUD_PROFILE|SANDBOX_PROVIDER|WUYING_MODE|WUYING_ROUTING|WUYING_ENDPOINT|WUYING_API_KEY|WUYING_DESKTOP_ID|WUYING_RELAY|WUYING_TUNNEL_PORT|WUYING_TUNNEL_KEY|WUYING_REGION_ID|WUYING_END_USER_ID)
      export "$key=$value"
      ;;
  esac
